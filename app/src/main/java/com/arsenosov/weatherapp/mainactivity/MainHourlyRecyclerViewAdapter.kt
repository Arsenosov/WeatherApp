	    ...
	    class HourlyViewHolder(private val view: View): RecyclerView.ViewHolder(view) {
	        fun bind(weather: HourlyWeatherSummary) {
                Glide.with(view)
                        ...
                        .into(view.ivHourlyWeather)
                view.tvHourlyTemperature.text = view.context.resources.getString(R.string.weather_temperature, weather.temp.roundToInt())
                view.tvHourlyTime.text = if (Date(weather.dt * 1000).after(Date())) DateFormat.format("HH:mm", Date(weather.dt * 1000))
